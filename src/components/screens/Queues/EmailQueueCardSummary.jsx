...
class EmailQueueCardSummary extends Component {
  state = {
    queue: 0,
    trend: "neutral",
  };
  componentDidMount() {
    ...
    if (!app.mySkillgroups)
      loadMySkillgroups(this, (result) => {
        if (!result.error) {
          if (!app.myEmailQueues)
            loadMyEmailQueues(this, (result) => {
              if (!result.error) this.prepareData();
            });
          else this.prepareData();
        }
      });
    else if (!app.myEmailQueues)
      loadMyEmailQueues(this, (result) => {
        if (!result.error) this.prepareData();
      });
    else this.prepareData();
  ...
  prepareData = () => {
    ...
    let queue = 0;
    if (app.myEmailQueues) {
      app.myEmailQueues.forEach((emailQueue) => {
  ...
  getQueue = (skillId) => {
    const { app } = this.props;
    if (app.myEmailQueues) {
      const targetQueues = app.myEmailQueues.filter(
...
EmailQueueCardSummary.propTypes = {
...
export default withStyles(styles, { withTheme: true })(EmailQueueCardSummary);
